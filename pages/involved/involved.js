...
Page({
    ...
    //ty_do 这里是再次点击我发起的的 时候重新获取数据渲染当if(a==0) 时 一共3个tab
    tabClick: function (e) {
      //需要授权才能操作
      if (app.globalData.userid == null) {
        wx.showToast({
      ...
      if(a == 0){
        ...
        wx.request({
          ...
          success: function (res) {
            ...
            //处理时间
            // var votelist = res.data; //old
            var votelist = res.data.vote_list_join;//new 
        ...
        wx.request({
          ...
          data: {
            ...
            //处理时间
            // var votelist = res.data;//old
            var votelist = res.data.get_vote_list_audit;//new 
            for (let i = 0; i < votelist.length; i++) {
              votelist[i].starttime = app.processTime(votelist[i].starttime);
              votelist[i].endtime = app.processTime(votelist[i].endtime);
            }
